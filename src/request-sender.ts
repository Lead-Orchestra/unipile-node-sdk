...
import {
	...
	ValidatorMissingError,
} from './errors/index.js';
import type { ClientState } from './state.js';
import type { ClientOptions } from './types/client.js';
import type { DefaultHeaders, RequestInput, RequestUrl } from './types/request.js';
...
export class RequestSender {
	...
	public async send<PayloadType>(input: RequestInput): Promise<PayloadType> {
		...
		if (options.logRequestResult ?? this.clientState.logRequestResult)
			console.log(
				`RequestSender: ${response.status}, ${response.statusText} - ${input.method} ${url}`
			);
		...
		const validationActivated =
			options.validateRequestPayload ?? this.clientState.validateRequestPayload;
		const validationLevel =
			options.validateRequestPayloadLevel ?? this.clientState.validateRequestPayloadLevel;
