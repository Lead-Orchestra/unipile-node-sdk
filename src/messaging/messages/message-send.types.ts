...
import type { OptionalToUnionUndefined } from '../../common/core.types.tmp.js';
import { BufferType, ReadableType } from '../../mails/attachments/resource.types.js';
import type { MessageDraftAttachment } from './ressource.types.js';
 ...
 *
 */
export const SendMessageToChatBodyValidator = TypeCompiler.Compile(SendMessageToChatBodySchema);
...
// );
export const AttachmentsPostQueryValidator = TypeCompiler.Compile(
	Type.Optional(Type.Array(AttachmentPostQuerySchema, { maxItems: MAX_ATTACHMENTS }))
);
...
export const FilesPostQueryValidator = TypeCompiler.Compile(
	Type.Object({
		...
		voice_message: Type.Optional(Type.Array(AttachmentPostQuerySchema, { maxItems: 1 })),
	})
);
