...
describe('extra_params', () => {
	...
	describe('GET', () => {
		//--------------------------------------------------------------------------
		it('should inject extra_params as query string ' +
			'on GET ' +
			'when no input params', async () => {
			const r = await client.account.getOne('some_account_id', {
				extra_params: { xtra: 'test' },
			});
			//   console.log(r);
		...
		it('should inject extra_params as query string ' +
			...
			'on GET ' +
			'when mixed with input params', async () => {
			const r = await client.account.getAll(
				{ limit: 30 },
				{ extra_params: { limit: 'garbage', legit: 'test' } }
			);
			//   console.log(r);
	...
	describe('POST | PUT', () => {
		//--------------------------------------------------------------------------
		it.skip('should inject extra_params in body ' +
			'on POST ' +
			'when no input params', async () => {});
		//--------------------------------------------------------------------------
		it('should inject extra_params in body ' +
			'and not overwrite input params ' +
			'on POST ' +
			'when body Content-Type is application/json ' +
			'and mixed with input params', async () => {
			...
			expect(fetchMock.mock.calls[0]?.[1]?.body).toBe(
				JSON.stringify({ provider: 'WHATSAPP', legit: 'test' })
			);
		});
		//--------------------------------------------------------------------------
		it('should inject extra_params in body ' +
	...
	describe('DELETE', () => {
		...
		it('should inject extra_params as query string ' +
			'on DELETE ' +
			'when no input params', async () => {
			const r = await client.account.delete('some_account_id', {
				extra_params: { xtra: 'test' },
			});
			//   console.log(r);
			expect((r as any).url).toBe(API_URL + 'accounts/some_account_id?xtra=test');
		...
		it('should inject extra_params as query string ' +
			...
			// console.log(r);
			expect((r as any).url).toBe(
				API_URL + 'emails/email-id?account_id=some_account_id&legit=test'
			);
		});
	});
});
