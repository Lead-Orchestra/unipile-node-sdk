...
describe('UserResource', () => {
	...
	describe('getProfile', () => {
		//--------------------------------------------------------------------------
		it.only(
			'should return UserProfile for any account provider' + 'on getProfile ' + 'when identifier',
			async () => {
				try {
	...
	describe('cancelInvitationSent', () => {
		...
		it(
			...
			async () => {
				...
				const invitation_id = (await client.users.getAllInvitationsSent({ account_id })).items[0]
					.id;
